...
namespace Hemrika.SharePresence.Common.ServiceLocation
{
    ...
    public class TypeMapping
    {
        ...
        public TypeMapping(Type typeFrom, Type typeTo, string key)
        {
            ...
            {
                throw new ArgumentException(string.Format(CultureInfo.CurrentCulture, Resources.NonAbstractType, typeTo.Name));
            }
            else if (!typeTo.IsSubclassOf(typeFrom) && typeTo.GetInterface(typeFrom.Name) == null)
            {
                throw new ArgumentException(string.Format(CultureInfo.CurrentCulture, Resources.ImplicitConversionNotDefined, typeTo.Name, typeFrom.Name));
            }
