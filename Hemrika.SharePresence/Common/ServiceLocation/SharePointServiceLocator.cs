...
namespace Hemrika.SharePresence.Common.ServiceLocation
{
    ...
    public class SharePointServiceLocator
    {
            ...
            if (serviceLocatorInstance == null)
            {
                lock (syncRoot)
                {
                    if (serviceLocatorInstance == null)
                    {
                        ...
                        {
                            throw new NoSharePointContextException(Resources.InvalidRunContext);
                        }
        ...
        /// <summary>
        /// Make sure that the <see cref="WebPartServiceLocator.Current"/> from the common service locator library doesn't work.
