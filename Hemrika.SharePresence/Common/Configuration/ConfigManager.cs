...
namespace Hemrika.SharePresence.Common.Configuration
{
    ...
    public class ConfigManager : IConfigManager
    {
        ...
        private static readonly string hemrikaKeyNamespace = "Hemrika.Config.Key";
        ...
        /// </summary>
        static public string HemrikaKeyNamespace
        {
            get
            {
                return hemrikaKeyNamespace;
            }
        }
        ...
        static public int MaxKeyLength
        {
            get
            {
                return maxKeyLength - hemrikaKeyNamespace.Length - SPSitePropertyBag.KeySuffix.Length;
            }
        }
        ...
        public TValue Get<TValue>(string key, IPropertyBag propertyBag)
        {
            ...
            catch (Exception ex)
            {
                string errorMessage = string.Format(CultureInfo.CurrentCulture
                                                    ,
                                                    Resources.InvalidConfigSetting
                                                    , key
                                                    , configValueAsString
                                                    , typeof(TValue)
                                                    , ex.GetType().FullName
        ...
        protected virtual void ValidateKey(string key)
        {
            ...
            if (key.StartsWith(HemrikaKeyNamespace, StringComparison.Ordinal))
            {
                string errorMessage = string.Format(CultureInfo.CurrentCulture,
                                                    Resources.InvalidKeyPrefix,
                                                    key, HemrikaKeyNamespace);
                throw new ConfigurationException(errorMessage);
            }
            else if (key.Length > MaxKeyLength || key.Length == 0)
            {
                string errorMessage = string.Format(CultureInfo.CurrentCulture,
                                                   Resources.InvalidKeyLength,
        ...
        {
            return HemrikaKeyNamespace + "." + key;
        }
