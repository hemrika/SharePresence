...
namespace Hemrika.SharePresence.Google.Analytics
{
  ...
  public class AccountEntry : AbstractEntry
  {
    ...
    public string AccountName
    {
        get
        {
            return FindPropertyValue("ga:accountName");
        }
    }

    public WebPropertiesQuery[] WebProperties
    {
        get
        {
            WebPropertiesQuery[] webproperties = null;
            List<AtomLink> webs = Links.Where(x => x.Rel.EndsWith("#child")).ToList<AtomLink>();
            if (webs.Count > 0)
            {
                List<WebPropertiesQuery> queries = new List<WebPropertiesQuery>();
                foreach (AtomLink link in webs)
                {
                    WebPropertiesQuery query = new WebPropertiesQuery(link.HRef.Content);
                    queries.Add(query);
                }
                webproperties = queries.ToArray<WebPropertiesQuery>();
            }
