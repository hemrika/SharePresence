...
namespace Hemrika.SharePresence.Google.Analytics
{
	...
	public class AnalyticsService : Service
	{
        ...
        /// <param name="feedQuery"></param>
        /// <returns>AccountFeed</returns>
        public WebPropertiesFeed Query(WebPropertiesQuery feedQuery)
        {
            return base.Query(feedQuery) as WebPropertiesFeed;
        }
        ...
        /// <param name="feedQuery"></param>
        /// <returns>AccountFeed</returns>
        public ProfilesFeed Query(ProfilesQuery feedQuery)
        {
            return base.Query(feedQuery) as ProfilesFeed;
        }
		...
		protected void OnNewFeed(object sender, ServiceEventArgs e)
		{
			...
			if (e.Uri.AbsolutePath.IndexOf("management/accounts") != -1)// == accountUri.AbsolutePath)//.IndexOf("/analytics/feeds/accounts/") != -1)
			{
                Uri accountUri = new Uri(AccountQuery.HttpsFeedUrl);
                if (e.Uri.AbsolutePath == accountUri.AbsolutePath)
                {
                    e.Feed = new AccountFeed(e.Uri, e.Service);
                    return;
                }

                //https://www.googleapis.com/analytics/v2.4/management/accounts/9934149/webproperties
                if (e.Uri.AbsolutePath.EndsWith("/webproperties"))
                {
                    e.Feed = new WebPropertiesFeed(e.Uri, e.Service);
                    return;
                }
                //https://www.googleapis.com/analytics/v2.4/management/accounts/9934149/webproperties/UA-9934149-1/profiles
                if (e.Uri.AbsolutePath.EndsWith("/profiles"))
                {
                    e.Feed = new ProfilesFeed(e.Uri, e.Service);
                    return;
                }
